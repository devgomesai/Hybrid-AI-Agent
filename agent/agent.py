...
async def web_search(
    ...
    This tool searches the web using Perplexity AI for current information, news,
    and real-time data that may not be in the local knowledge base. Best for
    getting up-to-date information, current events, and recent developments.

    Args:
        query: Search query for web search
        max_tokens: Maximum tokens in response (256-4096)
    ...
    Returns:
        Web search results with answer, citations, and metadata
    """
    ...
    return await web_search_tool(input_data)
...
async def web_search_with_context(
    ...
    """
    Search the web with conversation context for follow-up questions.

    This tool maintains conversation context for follow-up web searches,
    allowing for more contextual and relevant results based on previous
    conversation. Best for follow-up questions and contextual searches.

    Args:
        query: Search query for contextual web search
        context_messages: Previous conversation messages for context
        max_tokens: Maximum tokens in response (256-4096)

    Returns:
        Contextual web search results with answer and citations
    """
    if context_messages is None:
        context_messages = []

    input_data = PerplexityContextSearchInput(
        query=query,
        context_messages=context_messages,
        max_tokens=max_tokens
    )

    return await web_search_with_context_tool(input_data)